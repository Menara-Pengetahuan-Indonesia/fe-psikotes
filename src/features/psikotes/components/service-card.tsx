    ...
    >
      ...
      <CardHeader className="relative z-10 pt-7 px-7">
        <div className="flex items-start justify-between">
          ...
          {tag && (
            <Badge className="bg-white text-primary-700 border-slate-100 hover:bg-primary-50 rounded-full px-3 py-1 font-bold text-[10px] uppercase tracking-wider transition-colors shadow-sm">
              {tag}
            </Badge>
          )}
        </div>
      ...
      <CardContent className="relative z-10 space-y-3 mt-4 flex-1 px-7 pb-2">
        <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-tight group-hover:text-primary-700 transition-colors duration-300">
          {title}
        </h3>
      ...
      <CardFooter className="relative z-10 p-7 pt-2 flex items-center justify-between gap-4">
        {price && (
          <div className="flex flex-col">
            ...
            <p className="text-2xl font-black text-accent-500">{price}</p>
          </div>
        )}
