    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl space-y-6 text-center">
        <div className={cn(
          ...
          allAnswered ? 'bg-emerald-100' : 'bg-amber-100'
        )}>
          {allAnswered ? (
        ...
        <div className="px-4">
          ...
          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={cn('h-full rounded-full transition-colors', allAnswered ? 'bg-emerald-500' : 'bg-amber-500')}
        ...
        <div className="flex gap-3 pt-2">
          <button
            onClick={onCancel}
            disabled={submitting}
            className="flex-1 h-12 bg-white border border-slate-200 text-slate-900 rounded-xl font-black text-sm hover:bg-slate-50 transition-colors disabled:opacity-50"
          >
            Kembali
          </button>
          <button
            ...
            disabled={submitting}
            className="flex-1 h-12 bg-primary-600 text-white rounded-xl font-black text-sm hover:bg-primary-700 transition-colors shadow-lg active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2"
          >
            {submitting ? <><Loader2 className="size-4 animate-spin" /> Mengirim...</> : 'Ya, Kirim'}
          </button>
        </div>
      </div>
    </div>
