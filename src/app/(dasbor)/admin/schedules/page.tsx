...
const statusConfig: Record<ScheduleStatus, { label: string; badge: string; iconBg: string }> = {
  active: { label: 'Aktif', badge: 'bg-primary-50 text-primary-600', iconBg: 'from-primary-400 to-primary-500' },
  upcoming: { label: 'Mendatang', badge: 'bg-primary-50 text-primary-600', iconBg: 'from-primary-400 to-primary-500' },
                ...
                <div className="hidden lg:flex items-center gap-3 shrink-0">
                  <div className="w-32">
                    ...
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={cn('h-full rounded-full transition-all', schedule.status === 'active' ? 'bg-primary-500' : schedule.status === 'upcoming' ? 'bg-primary-500' : 'bg-slate-400')}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
